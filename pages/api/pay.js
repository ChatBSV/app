...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!authToken) {
      return res.status(401).json({ error: getErrorMessage(new Error('Expired authorization. Please reconnect to Handcash.')), redirectUrl: new HandCashService().getRedirectionUrl() });
    }
